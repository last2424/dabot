import deviantart, mysql.connector, sys, sshtunnel, time, re, random
...
class Bot:
    ...
    def buy_messages(self, comments):
        for comment in comments:
            ...
            if (comment.replies == 0 and comment.hidden == None):
                ...
                for stroke in list:
                    stroke = re.sub('</span>|<span>|<b>|</b>|<hr>|<hr />|</a>|</hr>|<sub>|</sub>|<i>|</i>', '', stroke)
                    ...
                    for row_user in self.cursor:
                        ...
                        if (self.cursor.rowcount > 0):
                            for row_item in self.cursor:
                                ...
                                self.cursor.execute(sql)
                                if (self.cursor.rowcount > 0):
                                    for row_inventory in self.cursor:
    ...
    def check_users(self, username):
                ...
                if (self.cursor.rowcount == 0):
                    ...
                    for i, row in enumerate(self.cursor):
                            try:
                                user = self.da.get_users([row['username']])[0]
                                time.sleep(5)
                    ...
                    if (self.cursor.rowcount > 0):
                        for row_user in self.cursor:
                            if (row_user['balance'] > 0):
                                list = re.sub('</span>|<span>|<b>|</b>|<hr>|<hr />|</hr>|</a>|<sub>|</sub>|<i>|</i>', '', list)
                                ...
                                self.cursor.execute(sql)
                                if (self.cursor.rowcount > 0):
                                    for row_transaction in self.cursor:
    ...
    def create_reply_fountain(self, commands_result, meanings, comment_id):
        ...
        for i in range(len(commands_result)):
            if (commands_result[i] == '0'):
                ...
                if (hr == ""):
                    hr = "<hr><small>Your current balance: <b>" + str(meanings[i][1]-1) + " NP</b><br><a href=" + str(
                        href) + ">Check your profile</a></small>"
                hr_hr = ""
                if (bot_add != ""):
                    hr_hr = "<hr />"
    ...
    def fountain_db(self, commands_result, meanings):
        for i in range(len(commands_result)):
            if(commands_result[i] == '0'):
                sql = ("UPDATE users SET balance=balance-1 WHERE username='" + str(meanings[i][2][3]) + "'")
                self.cursor.execute(sql)
                sql = ("INSERT INTO transaction (username, count, note, date) VALUES ('" + str(meanings[i][2][3]) + "', '-1', 'Magic Fountain', now())")
                self.cursor.execute(sql)
                sql = ("UPDATE users SET balance=balance+'" + str(meanings[i][2][2]) + "' WHERE username='" + str(meanings[i][2][3]) + "'")
                self.cursor.execute(sql)
                ...
                for row_item in meanings[i][2][1]:
                    ...
                    if (self.cursor.rowcount == 0):
                        ...
                        for row_inventory in self.cursor:
                            ...
                            self.cursor.execute(sql)
                    sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + meanings[i][2][3] + "', '" + self.items[i] + "', '1', 'Magic Fountain', now())")
                    self.cursor.execute(sql)
                ...
                for stroke in list:
                    ...
                    if (self.cursor.rowcount <= 0):
                        ...
                        if(user_exist == 1):
                            ...
                            meanings.append([g])
                            continue
                    for row_user_recipient in self.cursor:
                        sql = ("SELECT * FROM users WHERE username='" + str(comment.user.username) + "'")
                        self.cursor.execute(sql)
                        for row_user in self.cursor:
    ...
    def transfer(self, commands_result, meanings):
        for i in range(len(commands_result)):
            if (commands_result[i] == '0'):
                ...
                self.cursor.execute(sql)
                for last_transfer in self.cursor:
    ...
    def save_check(self, name, f_time, s_time, result, name2=""):
        ...
        form = form + "---------------------------------------------------------------------------------------------\n"
        form = form + "[" +str(datetime.today())+ "]\n"
        form = form + "Start check user with name " + name + " "+  f_time + "\n"
        if(result == 0):
            form = form + "Result is not found! "+ s_time +"\n"
        if(result == 1):
            form = form + "Result is found. " + name2 + "is " + name + " " + s_time + "\n"
        form = form + "---------------------------------------------------------------------------------------------\n"
