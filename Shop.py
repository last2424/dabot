        ...
        if (type == 1):
            for i in range(len(commands_result)):
                if (commands_result[i] == '0'):
                    is_hr = True
                    href = '"https://torimori.info/index.php?page=balance_info&id=' + str(meanings[i][0]) + '"'
                    sum = sum + meanings[i][3]*meanings[i][4]
                    hr = "<small>Your current balance: " + str(meanings[i][1]) + " NP<br><a href=" + str(href) + ">Check your profile</a></small>"
                    hr_hr = ""
                    if(bot_add != ""):
                        hr_hr = "<hr />"
                ...
                if (commands_result[i] == '1'):
                    ...
                    if (bot_add != ""):
                        hr_hr = "<hr />"
                    bot_add = bot_add + hr_hr + ":bulletorange: Thank you, bud! I'll ta- ... <i>stop!</i><br>You have no "+str(meanings[i][2])+" x"+str(meanings[i][3])+" in your inventory :<i></i>("
                if (commands_result[i] == '2'):
                    hr_hr = ""
                    if (bot_add != ""):
                        hr_hr = "<hr />"
        ...
        for i in range(len(comments)):
            ...
            if (comments[i].replies == 0 and comments[i].hidden == None):
                ...
                for j in range(len(list)):
                    list[j] = re.sub('</span>|<span>|<b>|</b>|<hr>|<hr />|</a>|</hr>|<i>|</i>|<sub>|</sub>', '', list[j])
                            ...
                            if (cursor.rowcount > 0):
                                for (count) in cursor:
                                    ...
                                    if(count[0]-c >= 0):
                                        ...
                                        sql = ("SELECT user_id, balance FROM users WHERE username='" + comments[
                                            i].user.username + "'")
                                        cursor.execute(sql)
                                        for (user_id, balance) in cursor:
                                            user_id = user_id
    ...
    def message_buy(self, comments):
       for i in range(len(comments)):
           ...
           if (comments[i].replies == 0 and comments[i].hidden == None):
                ...
                for j in range(len(list)):
                    list[j] = re.sub('</span>|<span>|<b>|</b>|<hr>|<hr />|</a>|</hr>|<sub>|</sub>|<i>|</i>', '', list[j])
                    list[j] = re.sub('<a\s(?<=<a\s).*?(?=>)>', '', list[j])
                    list[j] = re.sub('&nbsp;', ' ', list[j])
