    ...
    def create_reply(self, commands_result, meanings, comment_id):
        ...
        for i in range(len(commands_result)):
            ...
            if (commands_result[i] == '1'):
                is_hr = True
                href = '"http://last2424.ru/druid/index.php?page=balance_info&id=' + str(meanings[i][0]) + '#tab/one"'
        ...
        if (is_hr):
            bot_add = bot_add + hr
        self.da.post_comment(target='065C0C9A-FD63-A0DE-A801-FF8AFAB9F418', comment_type='deviation', body=bot_add, commentid=comment_id)
        time.sleep(5)


    def message(self):
       comments = self.da.get_comments(endpoint='deviation', deviationid='065C0C9A-FD63-A0DE-A801-FF8AFAB9F418')
                ...
                if (comments[i].replies == 0):
                    ...
                    if(cursor.rowcount > 0):
                        for j in range(len(list)):
                            list[j] = re.sub('</span>|<span>|<b>|</b>|<hr>|<hr />|</hr>|</a>', '', list[j])
                            ...
                            if (cursor.rowcount > 0):
                                ...
                                if(balance - c >= 0):
                                    ...
                                    sql = ("INSERT INTO transaction (username, count, note, transfer_id, date) VALUES ('" + str(comments[i].user.username) + "', '-" + str(c) + "', 'Transfer "+href+". Sending.', '" + str(t_id) + "', now())")
                                    cursor.execute(sql)
                                    href = '<a href="http://last2424.ru/index.php?page=balance_info&id='+str(user_id2)+'">'+str(g)+'</a>'
                                    sql = ("INSERT INTO transaction (username, count, note, transfer_id, date) VALUES ('" + str(g) + "', '" + str(c) + "', 'Transfer "+href+". Receiving.', '" + str(t_id) + "', now())")
                                    cursor.execute(sql)
