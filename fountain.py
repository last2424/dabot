...
class Fountain:
            ...
            for i in range(len(comments)):
                ...
                if (comments[i].replies == 0 and comments[i].hidden == None):
                    ...
                    if(list != None):
                        ...
                        print(meanings + " " + comments[i].user.username + " " + commands_result)
    ...
    def create_reply(self, commands_result, meanings, comment_id):
        ...
        if(self.is_admin == True):
            bot_add = '<b><font color="red">Don\'t worry. I\'m Admin.</font></b>' + bot_add
        self.da.post_comment(target='E4BD00A0-98EC-65C7-0BFC-43C2833B498F', comment_type='deviation', body=bot_add, commentid=comment_id)
        time.sleep(5)
            ...
            if(randChange <= self.chances[self.items[i]]):
                ...
                if(cursor.rowcount > 0):
                    for id, thumb in cursor:
                        sql = ("SELECT id FROM inventory WHERE item_id='" + str(id) + "' AND owner='"+str(username)+"'")
                        cursor.execute(sql)
                        if(cursor.rowcount == 0):
