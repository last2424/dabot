...
class Shop:
        ...
        if(type == 0):
            for i in range(len(commands_result)):
                if(commands_result[i] == '0'):
                    is_hr = True
                    href = '"https://torimori.info/druid/index.php?page=balance_info&id='+str(meanings[i][0])+'#tab/one"'
            ...
            if(is_hr):
                bot_add = bot_add + hr
            self.da.post_comment(target='99085360-CEEE-BA80-91AF-7B2EE2F19D3F', body=bot_add, comment_type='deviation', commentid=comment_id)
            time.sleep(5)
    ...
    def create_reply_sell(self, type, commands_result, meanings, comment_id):
        ...
        if (type == 1):
            ...
            if (is_hr):
                bot_add = bot_add + hr
            self.da.post_comment(target='99085360-CEEE-BA80-91AF-7B2EE2F19D3F', comment_type='deviation', body=bot_add, commentid=comment_id)
            time.sleep(5)
                            ...
                            if(type == 0):
                                ...
                                for (count) in cursor:
                                    ...
                                    if (cursor.rowcount > 0 and count[0] > 0):
                                        ...
                                        sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + comments[i].user.username + "', '" + str(item_name) + "', '" + str(c) + "', 'Sold out on dA', now())")
                                        cursor.execute(sql)
                            ...
                            if(type == 0):
                                ...
                                for balance in cursor:
                                    ...
                                    if(balance-(cost*c) >= 0):
                                        ...
                                        sql = ("UPDATE users SET last_transaction_time=now() WHERE username='" +comments[i].user.username + "'")
                                        cursor.execute(sql)
                                        sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('"+comments[i].user.username+"', '"+str(item_name)+"', '"+str(c)+"', 'Purchased on dA', now())")
                                        cursor.execute(sql)
                                        commands_result.append('0')
                                        sql = ("SELECT user_id, balance FROM users WHERE username='"+comments[i].user.username+"'")
                                        cursor.execute(sql)
