...
#Open Chest
    ...
    def bagRandomEvent(self):
        self.backpacks = ["Lica Candy Bag", "Blind Mikazu Candy Bag", "Toxic Mimitoka Candy Bag", "Skeleton Mocca Candy Bag", "Fallen Torimori Candy Bag"];
        self.items_backpack = {
            "Lica Candy Bag": [
                                "Lica Candy - Mint",
                                "Lica Candy - Lemon",
                                "Lica Candy - Strawberry",
                                "Lica Candy - Cocoa",
                                "Lica Candy - Watermelon",
                                "Lica Candy - Orange",
                                "Shadow Potion M",
                                "Shadow Potion S",
                                "Striped Caramel Lollipop",
                                "Shoco Choco Bar",
                                "Halloween mini Cake",
                                "Chipwith Ketchup",
                                "Candy Corn",
                                "Orange Swirl Candy",
                                "Red and Black striped candy",
                                "Zip Pop Candy",
                                "Pumpkin Candy",
                                "Blue Striped Candy",
                                "Purple Swirl Lollipop",
                                "Black and Orange striped lollipop",
                                "Koi Jellybean",
                                "Sugar Cube",
                                "Rainbow Sparkle Lollipop"],
            "Blind Mikazu Candy Bag": [
                                "Ancient Potion M",
                                "Ancient Potion S",
                                "Radish Candy",
                                "Bitten Mikazu's Candy",
                                "Striped Caramel Lollipop",
                                "Shoco Choco Bar",
                                "Halloween mini Cake",
                                "Chip with Ketchup",
            ...
            "Skeleton Mocca Candy Bag": [
                                ...
                                "Sugar Cube",
                                "Rainbow Sparkle Lollipop"],
            "Fallen Torimori Candy Bag": [
                                "Torimori Lollipop",
                                "Pink Jelly Torimori",
                                "Light Potion M",
                                "Light Potion S",
                                "Purple Orange Jelly Worm",
                                "Angelic Wings",
                                "Striped Caramel Lollipop",
                                "Shoco Choco Bar",
                                "Halloween mini Cake",
        ...
        if (self.server.is_active == True):
            comments = self.da.get_comments(endpoint='deviation', deviationid='0012145A-ABE3-5A86-5496-48FF6CE0C9AA', limit=25)
        else:
            self.ssh_reconnect()
            comments = self.da.get_comments(endpoint='deviation', deviationid='0012145A-ABE3-5A86-5496-48FF6CE0C9AA', limit=25)
        time.sleep(60)
        ...
        for comment in comments:
            if (comment.replies == 0 and comment.hidden == None):
                self.check_users(comment.user.username)
        self.bagMessageInit(comments)

    def bagMessageInit(self, comments):
        for comment in comments:
            commands_result = []
            meanings = []
            comment.body = self.tags(comment.body)
            if (comment.replies == 0 and comment.hidden == None):
                list = None
                backpack = None
                for l in self.backpacks:
                    list = re.search('^Open '+l+'$', comment.body, re.IGNORECASE)
                    if (list != None):
                        list = list.group(0)
                        backpack = l
                        break
                if backpack != None:
                    sql = ("SELECT * FROM items WHERE name='" + str(backpack) + "'")
                    self.cursor.execute(sql)
                    rows_backpack = self.cursor.fetchall()
                    v = 3
                    if backpack == "Fallen Torimori Candy Bag":
                        v = 5
                    for row_backpack in rows_backpack:
                        sql = ("SELECT * FROM inventory WHERE item_id='" + str(row_backpack['id']) + "' and owner='" + str(comment.user.username) + "'")
                        self.cursor.execute(sql)
                        rows_backpack_inv = self.cursor.fetchall()
                        if(self.cursor.rowcount > 0):
                            for row_backpack_inv in rows_backpack_inv:
                                if row_backpack_inv['count'] > 0:
                                    r = self.backpack_random(self.items_backpack[backpack], v)
                                    commands_result.append('0')
                                    meanings.append([comment.user.username, r, backpack])
                                else:
                                    commands_result.append('1')
                    self.create_reply_backpack(commands_result=commands_result, meanings=meanings, comment_id=comment.commentid)


    def backpack_random(self, i, v):
        answer = ["", []]
        random.shuffle(i)
        for j in range(v):
            sql = ("SELECT * FROM items WHERE name='" + str(i[j]) + "'")
            self.cursor.execute(sql)
            if (self.cursor.rowcount > 0):
                rows_item = self.cursor.fetchall()
                for row_item in rows_item:
                    answer[1].append(row_item)
                    answer[0] = answer[0] + row_item['thumb']
            else:
                print(row_item['name'])
        answer[0] = "You've found:<br>" + answer[0]
        return answer
    ...
    def create_reply_backpack(self, commands_result, meanings, comment_id):
        bot_add = ""
        for i in range(len(commands_result)):
            if(commands_result[i] == '0'):
                bot_add = bot_add + ":bulletgreen: "+meanings[i][1][0]
            if(commands_result[i] == '1'):
                bot_add = bot_add + ":bulletorange: <i>Oops, I didn't find this bag in your inventory! </i>"
        self.da.post_comment(target='0012145A-ABE3-5A86-5496-48FF6CE0C9AA', body=bot_add, comment_type='deviation', commentid=str(comment_id))
        time.sleep(5)
        self.backpack_db(commands_result, meanings)

    def backpack_db(self, commands_result, meanings):
        for i in range(len(commands_result)):
            if(commands_result[i] == '0'):
                sql = ("SELECT * FROM items WHERE name='" + str(meanings[i][2]) + "'")
                self.cursor.execute(sql)
                rows_item = self.cursor.fetchall()
                for row_item in rows_item:
                    sql = ("SELECT * FROM inventory WHERE item_id='"+str(row_item['id'])+"' AND owner='" + str(meanings[i][0]) + "'")
                    self.cursor.execute(sql)
                    rows_inv = self.cursor.fetchall()
                    for row_inv in rows_inv:
                        sql = ("UPDATE inventory SET count=count-1 WHERE id='" + str(row_inv['id']) + "'")
                        self.cursor.execute(sql)
                        sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + str(meanings[i][0]) + "', '" + str(row_item['name']) + "', '-1', '"+str(meanings[i][2])+"', now())")
                        self.cursor.execute(sql)
                for row in range(len(meanings[i][1][1])):
                    sql = ("SELECT * FROM inventory WHERE item_id='"+str(meanings[i][1][1][row]['id'])+"' AND owner='" + str(meanings[i][0]) + "'")
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount > 0):
                        rows_inventory = self.cursor.fetchall()
                        for row_inventory in rows_inventory:
                            sql = ("UPDATE inventory SET count=count+1 WHERE id='" + str(row_inventory['id']) + "'")
                            self.cursor.execute(sql)
                            sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + str(meanings[i][0]) + "', '" + str(meanings[i][1][1][row]['name']) + "', '1', '"+str(meanings[i][2])+"', now())")
                            self.cursor.execute(sql)
                    else:
                        sql = ("INSERT INTO inventory (item_id, owner, count) VALUES ('" + str(meanings[i][1][1][row]['id']) + "', '" + str(meanings[i][0]) + "', '1')")
                        self.cursor.execute(sql)
                        sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + str(meanings[i][0]) + "', '" + str(meanings[i][1][1][row]['name']) + "', '1', '"+str(meanings[i][2])+"', now())")
                        self.cursor.execute(sql)
