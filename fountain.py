...
class Fountain:
    def __init__(self, da, cnx, cursor):
        self.items = ["Shiny Coin", "Refresh Water M", "Potion of Change", "Ancient Potion M", "Fur Potion M", "Cotton Potion M", "Sage Potion M", "Prismatic Potion M", "Auta Potion M"]
        self.chances = {
            "Shiny Coin": 1,
            "Refresh Water M": 7,
            "Potion of Change": 2,
    ...
    def create_reply(self, commands_result, meanings, comment_id):
        ...
        for i in range(len(commands_result)):
            if (commands_result[i] == '0'):
                is_hr = True
                href = '"https://torimori.info/druid/index.php?page=balance_info&id=' + str(meanings[i][0]) + '"'
                if (hr == ""):
        ...
        if (is_hr):
            bot_add = bot_add + hr
        self.da.post_comment(target='065C0C9A-FD63-A0DE-A801-FF8AFAB9F418', comment_type='deviation', body=bot_add,
