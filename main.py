...
class Bot:
    ...
    def forest_ivent(self):
        self.items_forest = ["Shiny Coin", "Refresh Water M", "Potion of Change", "Ancient Potion M", "Beast Potion M",
                      "Tailor Potion M", "Sage Potion M", "Prismatic Potion M", "Aura Potion M"]
        self.chances_forest = {
            "Shiny Coin": 2,
            "Refresh Water M": 15,
            "Potion of Change": 7,
    ...
    def forest_messages(self, comments):
        for comment in comments:
            ...
            meanings = []
            if (comment.replies == 0 and comment.hidden == None):
                list = re.search('Explore\sThe\sForest', comment.body, re.IGNORECASE)
                if (list != None):
                    list = list.group(0)
                    sql = ("SELECT * FROM users WHERE username='" + str(comment.user.username) + "'")
                    self.cursor.execute(sql)
                        ...
                        for row_user in rows_user:
                            ...
                            if(transaction_date != None):
                                ...
                                if (datetime.now().date() - date(int(transaction_date[0]), int(transaction_date[1]), int(transaction_date[2]))).days >= 1:
                                    current_answers = self.forest_random(username=comment.user.username)
                                    commands_result.append('0')
                                    meanings.append([row_user['user_id'], row_user['balance'], current_answers])
                                else:
                                    commands_result.append('2')
    ...
    def forest_random(self, username):
        answer = ["", []]
        answer.append(username)
        answer.append([])
        for i in range(len(self.items_forest)):
            j = 0
            randCount = 0
            while j < 10:
                randChange = 1 + random.random() * 99
                if (randChange <= self.chances_forest[self.items_forest[i]]):
                    sql = ("SELECT * FROM items WHERE name='" + str(self.items_forest[i]) + "'")
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount > 0):
                        rows_item = self.cursor.fetchall()
                        for row_item in rows_item:
                            randCount += 1
                if(j == 9 and randCount > 0):
                    answer[1].append(row_item)
        ...
        for i in range(len(commands_result)):
            if(commands_result[i] == '0'):
                ...
                for j, row_item in enumerate(meanings[i][2][1]):
                    ...
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount == 0):
                        sql = ("INSERT INTO inventory (item_id, owner, count) VALUES ('" + str(row_item['id']) + "', '" + meanings[i][2][2] + "', '" + meanings[i][2][3][j] + "')")
                        self.cursor.execute(sql)
                    else:
                        rows_inventory = self.cursor.fetchall()
                        for row_inventory in rows_inventory:
                            ...
                            self.cursor.execute(sql)
                    sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + meanings[i][2][2] + "', '" + row_item['name'] + "', '" + meanings[i][2][3][j] + "', 'Forest Exploring - Treasure Hunt Event', now())")
                    self.cursor.execute(sql)

    def shovel_ivent(self):
        self.items_shovel = ["Shiny Coin", "Refresh Water M", "Potion of Change", "Ancient Potion M", "Beast Potion M",
                      "Tailor Potion M", "Sage Potion M", "Prismatic Potion M", "Aura Potion M"]
        self.chances_shovel = {
            "Shiny Coin": 2,
            "Refresh Water M": 15,
    ...
    def shovel_messages(self, comments):
        for comment in comments:
            ...
            meanings = []
            if (comment.replies == 0 and comment.hidden == None):
                list = re.search('Use\sShovel', comment.body, re.IGNORECASE)
                if (list != None):
                    list = list.group(0)
                    sql = ("SELECT * FROM users WHERE username='" + str(comment.user.username) + "'")
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount > 0):
                        ...
                        for row_user in rows_user:
                            ...
                            list = re.sub('<span>', '', list)
                            list = re.sub('&nbsp;', ' ', list)
                            sql = ("SELECT * FROM inventory WHERE owner='"+str(comment.user.username)+"'")
                            self.cursor.execute(sql)
                            rows_inv = self.cursor.fetchall()
                            for i, row_inv in enumerate(rows_inv):
                                if row_inv['item_id'] == 112 and row_inv['count'] > 0:
                                    current_answers = self.shovel_random(username=comment.user.username)
                                    commands_result.append('0')
                                    meanings.append([row_user['user_id'], row_user['balance'], current_answers])
                                    break
                                else:
    ...
    def shovel_random(self, username):
        answer = ["", []]
        answer.append(username)
        answer.append([])
        for i in range(len(self.items_shovel)):
            j = 0
            randCount = 0
            while j < 10:
                randChange = 1 + random.random() * 99
                if (randChange <= self.chances_shovel[self.items_shovel[i]]):
                    sql = ("SELECT * FROM items WHERE name='" + str(self.items_shovel[i]) + "'")
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount > 0):
                        rows_item = self.cursor.fetchall()
                        for row_item in rows_item:
    ...
    def create_reply_shovel(self, commands_result, meanings, comment_id):
        ...
        for i in range(len(commands_result)):
            if (commands_result[i] == '0'):
                ...
                bot_add = bot_add + hr_hr + "<i>You used a shovel until it broke...</i><br><br><hr>"+href+" x1 has been removed from your inventory!<br><br>" + str(meanings[i][2][0])
            if (commands_result[i] == '2'):
                hr_hr = ""
                if (bot_add != ""):
                    hr_hr = "<hr />"
        ...
        for i in range(len(commands_result)):
            if(commands_result[i] == '0'):
                ...
                for j, row_item in enumerate(meanings[i][2][1]):
                    ...
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount == 0):
                        sql = ("INSERT INTO inventory (item_id, owner, count) VALUES ('" + str(row_item['id']) + "', '" + meanings[i][2][2] + "', '" + str(meanings[i][2][3][j]) + "')")
                        self.cursor.execute(sql)
                    else:
                        rows_inventory = self.cursor.fetchall()
                        for row_inventory in rows_inventory:
                            sql = ("UPDATE inventory SET count=count+" + str(meanings[i][2][3][j]) + " WHERE id='" + str(row_inventory['id']) + "'")
                            self.cursor.execute(sql)
                    sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + meanings[i][2][2] + "', '" + row_item['name'] + "', '" + str(meanings[i][2][3][j]) + "', 'Dig - Treasure Hunt Event', now())")
                    self.cursor.execute(sql)
                sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + meanings[i][2][2] + "', 'Shovel', '-1', 'Dig - Treasure Hunt Event', now())")
                self.cursor.execute(sql)
