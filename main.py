...
class Bot:
        ...
        for i in range(len(self.items)):
            ...
            if (randChange <= self.chances[self.items[i]]):
                ...
                if (self.cursor.rowcount > 0):
                    for row_item in self.cursor:
                        answer[1].append(row_item)
                        randCount = round(1 + random.random() * 5)
                        answer[3].append(randCount)
                        answer[0] = answer[0] + self.answers[self.items[i]] + row_item['thumb'] + " x"+randCount
        if (answer[0] == ""):
            answer[0] = answer[0] + self.answers['Nothing']
        return answer
    ...
    def create_reply_forest(self, commands_result, meanings, comment_id):
        ...
        for i in range(len(commands_result)):
            ...
            if (commands_result[i] == '2'):
                ...
                if (bot_add != ""):
                    hr_hr = "<hr />"
                bot_add = bot_add + hr_hr + "<i>You don't ready for another try!</i><br>Come back in a couple of days~"
    ...
    def forest_db(self, commands_result, meanings):
        for i in range(len(commands_result)):
            if(commands_result[i] == '0'):
                for i, row_item in enumerate(meanings[i][2][1]):
                    sql = ("SELECT * FROM inventory WHERE item_id='" + str(row_item['id']) + "' AND owner='" + str(meanings[i][2][2]) + "'")
                    self.cursor.execute(sql)
                    if (self.cursor.rowcount == 0):
                        sql = ("INSERT INTO inventory (item_id, owner, count) VALUES ('" + str(row_item['id']) + "', '" + meanings[i][2][2] + "', '" + meanings[i][2][3][i] + "')")
                        self.cursor.execute(sql)
                    else:
                        rows_inventory = self.cursor.fetchall()
                        for row_inventory in rows_inventory:
                            sql = ("UPDATE inventory SET count=count+" + meanings[i][2][3][i] + " WHERE id='" + str(row_inventory['id']) + "'")
                            self.cursor.execute(sql)
                    sql = ("INSERT INTO inventorylog (username, item, count, note, date) VALUES ('" + meanings[i][2][2] + "', '" + row_item['name'] + "', '" + meanings[i][2][3][i] + "', 'Forest', now())")
                    self.cursor.execute(sql)
    ...
    def shovel_messages(self, comments):
        for comment in comments:
            commands_result = []
            meanings = []
            list = re.search('Dig', comment.body, re.IGNORECASE)
            if (list != None):
                list = list.group(0)
                sql = ("SELECT * FROM users WHERE username='" + str(comment.user.username) + "'")
                self.cursor.execute(sql)
